...
// #                                                                             #
// #           Visual private key generator (c) 2020 by Ntcfinans                #
// #                                                                             #
// ###############################################################################
...
var cellfillcolour = "#42A5F5"; //cell colour for bit=1
var cellnofillcolour = "white"; //cell colour for bit=0
var cellblockcolour = "#42A5F5"; //cell colour if user blocks the whole line (all bits equals to 1), advanced option
...
// Function returns uncompressed pubkey, uncompressed address, compressed pubkey and compressed address
...
if (!/[^0123456789abcdef]+/i.test(HEXinput.value) && HEXinput.value != "" && HEXinput.value.length <=64 ) {
  ...
  HEXinput.style.background="#42A5F5";
  }
}
...
function rndPrivKey() {
for (var x=0; x<16; x++) {for (var y=12; y<16; y++) {
    if (blockX[x] == 0 && blockY[y] == 0) {BTCpk[y][x] = Math.floor(Math.random() * 2)};
    }}
BTCpk[11][14] = 1;
BTCpk[11][15] = Math.floor(Math.random() * 2);
if (!Quiet.checked) { fillAllfromArr(); }
calculation();
LastCell = [16, 16];
}

function leftShiftPrivKey() {
for (var y=12; y<16; y++) {
	var temp = BTCpk[y][0];
	for (var x=0; x<16; x++) {
	    if (blockX[x] == 0 && blockY[y] == 0) {
		if (x < 15) {
			BTCpk[y][x] = BTCpk[y][x+1]
		}
		else {
			BTCpk[y][x] = temp
		}
...
/*
function addOnePrivKey() {
var carry = 1;
for (var y=15; y>=12; y--) {for (var x=15; x>=0; x--) {
		if (BTCpk[y][x] == 1 && carry == 1) { BTCpk[y][x] = 0; carry = 1} else { 
			if (BTCpk[y][x] == 0 && carry == 1) { BTCpk[y][x] = 1; carry = 0; break} }
    }}
fillAllfromArr();
calculation();
LastCell = [16, 16];
...
for (var y=15; y>=12; y--) {
	for (var x=15; x>=0; x--) {
		if (BTCpk[y][x] == 1 && carry == 1) { BTCpk[y][x] = 0; carry = 1} else { 
			if (BTCpk[y][x] == 0 && carry == 1) { BTCpk[y][x] = 1; carry = 0; continua = false; break} }
	}
	if (!continua) {break}
}
...
function calculation() {
	...
	if (BTCaddr_c_gen.value.startsWith("13z")) 
	{ 
		document.getElementById("Resultados").value +=  BTCaddr_c_gen.value + " - " + BTChex.value.substring(45) + "\r\n"; 
		if (auto == null) { alert(BTCaddr_c_gen.value + "\r\n" + BTChex.value.substring(20)); }
		if (BTCaddr_c_gen.value.endsWith("5so")) { auto == null; alert(BTCaddr_c_gen.value + "\r\n" + BTChex.value.substring(20)); }
	}
...
xhr.onreadystatechange = function() {
  ...
  if (xhr.status === 200) {
     ...
     if (fieldForResult.value == "\u0243" + " " + "0") {fieldForResult.style.background="none"} else {fieldForResult.style.background="#42A5F5"};
     } else {
...
function AutoRandom () {
    if (auto === null) { auto = setInterval(function(){document.getElementById('RandButton').click()},1) } else { clearInterval(auto); auto = null; }
}

function AutoSecuencial () {
    if (auto === null) { auto = setInterval(function(){document.getElementById('SeqButton').click()},1) } else { clearInterval(auto); auto = null; }
}
...
document.getElementById("LeftShiftButton").addEventListener("click", leftShiftPrivKey, false);

document.getElementById("SeqButton").addEventListener("click", addOnePrivKey, false);

document.getElementById("Autom").addEventListener("click", AutoRandom, false);
document.getElementById("AutoSeq").addEventListener("click", AutoSecuencial, false);
